...
import { Loader2, AlertTriangle, ListChecks, MessageSquare, RefreshCw, ImageOff, ChevronDown, ChevronUp, Edit } from "lucide-react";
...
function MatchHistoryPage() {
      ...
      .then((versions) => {
        if (versions && versions.length > 0) {
          ...
          Promise.all(staticDataFetches)
            .then(([summonerData, runesData, champData]) => {
              const spells = {};
  ...
  useEffect(() => {
    if (totalPages === 1 && currentPage !== 1) {
      // If only one page total, current should be 1
      setCurrentPage(1);
    } else if (currentPage > totalPages && totalPages > 0) {
      // If current page is out of bounds
        ...
        {!isLoadingMatches && Object.keys(groupedMatches).length > 0 && (
          <div className="space-y-3 max-w-4xl mx-auto px-4 sm:px-6 md:px-8 pb-8">
            {Object.entries(groupedMatches).map(([dateKey, matchesOnDate]) => (
              <div key={dateKey}>
                <h2 className="text-lg font-semibold text-orange-400 mb-3 pb-1.5 border-b border-gray-700/80">{dateKey}</h2>
                  ...
                  {matchesOnDate.map((match) => {
                    ...
                    return (
                      <div key={match.matchId} className={`rounded-lg shadow-lg overflow-hidden group ${resultBgOverlayClass} ${animationClass}`}>
                                ...
                                <div className="flex space-x-0.5">
                                  {itemsRow1.map((itemSrc, idx) => (
                                    <div key={`item-r1-${idx}-${match.matchId}`} className="w-6 h-6 bg-gray-800 rounded flex items-center justify-center border border-gray-600/50">
                                      {" "}
                                      {itemSrc ? <img src={itemSrc} alt={`I${idx + 1}`} className="w-5 h-5 rounded-sm" /> : <div className="w-5 h-5 rounded-sm bg-gray-700"></div>}{" "}
                                    </div>
                                  ))}
                                ...
                                <div className="flex space-x-0.5">
                                  {itemsRow2.map((itemSrc, idx) => (
                                    <div key={`item-r2-${idx}-${match.matchId}`} className="w-6 h-6 bg-gray-800 rounded flex items-center justify-center border border-gray-600/50">
                                      {" "}
                                      {itemSrc ? <img src={itemSrc} alt={`I${idx + 4}`} className="w-5 h-5 rounded-sm" /> : <div className="w-5 h-5 rounded-sm bg-gray-700"></div>}{" "}
                                    </div>
                                  ))}
                        ...
                        {isExpanded && (
                          <ExpandedMatchDetails match={match} ddragonVersion={ddragonVersion} championData={championData} summonerSpellsMap={summonerSpellsMap} runesMap={runesMap} getChampionImage={getChampionImage} getSummonerSpellImage={getSummonerSpellImage} getItemImage={getItemImage} getRuneImage={getRuneImage} getChampionDisplayName={getChampionDisplayName} isTrackedPlayerWin={isWin} roleIconMap={ROLE_ICON_MAP} roleOrder={ROLE_ORDER} processTimelineDataForPlayer={processTimelineData} />
                        )}
                      </div>
                    );
                  })}
