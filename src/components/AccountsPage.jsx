...
function AccountsPage() {
  ...
  const handleAddAccount = async (e) => {
    ...
    try {
      const existingAccount = await db.trackedAccounts.where({ riotId: lowerCaseRiotId, platformId: selectedPlatformId }).first();
            ...
            <form onSubmit={handleAddAccount} className="space-y-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" id="riotIdInput" value={riotIdInput} onChange={(e) => setRiotIdInput(e.target.value)} placeholder="YourGameName#TAG" className="w-full pl-10 pr-4 py-3 bg-gray-800 border border-gray-700 rounded-lg shadow-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-gray-100 placeholder-gray-500 transition-colors" required disabled={isLoading || isFetching || !ddragonVersion} />
                </div>
              </div>
              ...
              <div>
                ...
                <select id="platformId" value={selectedPlatformId} onChange={(e) => setSelectedPlatformId(e.target.value)} className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg shadow-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-gray-100 transition-colors" disabled={isLoading || isFetching || !ddragonVersion}>
              ...
              <button type="submit" className="w-full bg-orange-600 hover:bg-orange-500 text-white font-semibold py-3.5 px-4 rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-offset-2 focus:ring-offset-gray-900 flex items-center justify-center transition-all duration-150 ease-in-out disabled:opacity-60 disabled:cursor-not-allowed" disabled={isLoading || isFetching || !ddragonVersion || !RIOT_API_KEY}>
            ...
            {isFetching ? (
              ...
              <ul className="space-y-4">
                {accounts.map((account) => {
                  ...
                  return (
                    <li key={account.id} className="bg-gray-800/70 border border-gray-700/60 p-3 rounded-lg shadow-lg flex items-center justify-between transition-all duration-200 ease-in-out hover:bg-gray-700/90 hover:shadow-xl hover:border-gray-600">
                      ...
                      <button onClick={() => handleRemoveAccount(account.id, displayRiotId)} className="text-gray-500 hover:text-red-500 transition-colors p-2 rounded-md hover:bg-red-500/10 disabled:opacity-50 flex-shrink-0 ml-3" aria-label={`Remove account ${displayRiotId}`} disabled={isLoading}>
                        <Trash2 size={18} />
                      </button>
                    </li>
                  );
