...
function TopNavbar({ currentView, setCurrentView }) {
  ...
  return (
    <nav className="w-full bg-black-800 fixed top-0 left-0 right-0 z-20">
        ...
        <div className="flex items-center justify-between h-[50px]">
          ...
          <div className="flex items-center space-x-1">
            {mainNavItems.map((item) => (
              <NavItem key={item.id} icon={item.icon} label={item.label} isActive={currentView === item.id} onClick={() => handleNavClick(item.id)} />
            ))}
          </div>

          {/* Right-aligned Account Link */}
          <div className="flex items-center">
            <NavItem key={accountNavItem.id} icon={accountNavItem.icon} label={accountNavItem.label} isActive={currentView === accountNavItem.id} onClick={() => handleNavClick(accountNavItem.id)} />
          </div>
        </div>
      </div>
    </nav>
