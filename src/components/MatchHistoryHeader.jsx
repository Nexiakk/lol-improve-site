...
const ChampionFilterWithSuggestions = ({
  ...
  const attemptDirectMatchAndSelect = () => {
    ...
    const directMatch = availableChampions.find(apiName => 
        normalizeText(getChampionDisplayName(apiName) || apiName) === normalizedInput || 
        normalizeText(apiName) === normalizedInput
    );
  ...
  return (
    <div className="relative h-9" ref={wrapperRef}> {/* Ensure parent has h-9 */}
      ...
      {showSuggestions && suggestions.length > 0 && (
        <ul className="absolute z-20 w-full bg-gray-700 border border-gray-600 rounded-md shadow-lg mt-1 max-h-60 overflow-y-auto py-1">
          {suggestions.map((champion, index) => (
            ...
            >
              <img src={getChampionImage(champion.apiName)} alt="" className="w-5 h-5 rounded-sm" onError={(e) => { e.target.style.display='none';}} />
...
const RoleFilter = ({ selectedRole, onRoleChange, commonInputClass, ROLE_ICON_MAP, ROLE_ORDER }) => {
    ...
    return (
        <div className={`flex items-center bg-gray-700/80 border border-gray-600 rounded-l-md shadow-sm h-9 px-1.5`}> {/* Ensure h-9 */}
...
const FilterInput = ({ label, icon: Icon, children, htmlFor }) => (
  <div className="flex-1 min-w-[150px]"> <label htmlFor={htmlFor} className="block text-xs font-medium text-gray-400 mb-1 flex items-center"> {Icon && <Icon size={14} className="mr-1.5 text-orange-400" />} {label} </label> {children} </div>
);
FilterInput.displayName = 'FilterInput';

// Styles for react-select
const selectStyles = { control: (p, s) => ({ ...p, backgroundColor: 'var(--color-gray-700, #4a5568)', borderColor: s.isFocused ? 'var(--color-orange-500, #ed8936)' : 'var(--color-gray-600, #718096)', boxShadow: s.isFocused ? '0 0 0 1px var(--color-orange-500, #ed8936)' : null, borderRadius: '0.375rem', minHeight: '36px', height: '36px', fontSize: '0.75rem', color: 'var(--color-gray-200, #e2e8f0)', '&:hover': { borderColor: 'var(--color-orange-500, #ed8936)' }, }), valueContainer: (p) => ({ ...p, padding: '0px 6px', height: '36px', }), input: (p) => ({ ...p, color: 'var(--color-gray-100, #f7fafc)', margin: '0px', paddingBottom: '0px', paddingTop: '0px', }), placeholder: (p) => ({ ...p, color: 'var(--color-gray-400, #a0aec0)', }), singleValue: (p) => ({ ...p, color: 'var(--color-gray-100, #f7fafc)', }), multiValue: (p) => ({ ...p, backgroundColor: 'var(--color-orange-600, #dd6b20)', borderRadius: '0.25rem', color: 'white', }), multiValueLabel: (p) => ({ ...p, color: 'white', fontSize: '0.75rem', padding: '2px', paddingLeft: '6px', }), multiValueRemove: (p) => ({ ...p, color: 'white', borderRadius: '0 0.25rem 0.25rem 0', '&:hover': { backgroundColor: 'var(--color-orange-700, #c05621)', color: 'white', }, }), menu: (p) => ({ ...p, backgroundColor: 'var(--color-gray-700, #4a5568)', borderRadius: '0.375rem', marginTop: '4px', zIndex: 20, }), menuList: (p) => ({ ...p, paddingTop: '0px', paddingBottom: '0px', }), option: (p, s) => ({ ...p, backgroundColor: s.isSelected ? 'var(--color-orange-600, #dd6b20)' : s.isFocused ? 'var(--color-orange-500-alpha20, rgba(237, 137, 54, 0.2))' : 'transparent', color: s.isSelected ? 'white' : 'var(--color-gray-200, #e2e8f0)', fontSize: '0.75rem', padding: '6px 10px', '&:active': { backgroundColor: 'var(--color-orange-700, #c05621)', }, }), indicatorSeparator: () => ({ display: 'none', }), dropdownIndicator: (p) => ({ ...p, padding: '4px', color: 'var(--color-gray-400, #a0aec0)', '&:hover': { color: 'var(--color-gray-200, #e2e8f0)', }, }), clearIndicator: (p) => ({ ...p, padding: '4px', color: 'var(--color-gray-400, #a0aec0)', '&:hover': { color: 'var(--color-red-400, #f87171)', }, }), loadingIndicator: (p) => ({ ...p, color: 'var(--color-orange-500, #ed8936)', }), };
...
function MatchHistoryHeader({
  ...
  const handleFilterDateChange = (dates) => { const [start, end] = dates; onFilterChange({ target: { name: 'dateRange', value: { startDate: start ? format(start, 'yyyy-MM-dd') : null, endDate: end ? format(end, 'yyyy-MM-dd') : null } } }); };
  const handleUpdateFetchDateRangeChange = (dates) => { const [start, end] = dates; onUpdateFetchDateChange({ target: { name: 'dateRange', value: { startDate: start ? format(start, 'yyyy-MM-dd') : null, endDate: end ? format(end, 'yyyy-MM-dd') : null } } }); };
  const handlePatchChange = (selectedOptions) => { const patchValues = selectedOptions ? selectedOptions.map(option => option.value) : []; onFilterChange({ target: { name: 'patch', value: patchValues } }); };
  const handleRoleFilterChange = (roleValue) => { onFilterChange({ target: { name: 'role', value: roleValue } }); };
  const isAnyFilterActive = Object.values(filters).some(val => { if (Array.isArray(val)) return val.length > 0; if (typeof val === 'object' && val !== null) return val.startDate || val.endDate || (val.role && val.role !== ''); return val && typeof val === 'string' && val !== ''; });
  const patchOptions = useMemo(() => availablePatches.map(p => ({ value: p, label: p })), [availablePatches]);
  const selectedPatchValues = useMemo(() => { if (Array.isArray(filters.patch)) return filters.patch.map(p => ({ value: p, label: p })); return []; }, [filters.patch]);
  useEffect(() => { function handleClickOutside(event) { if (advancedUpdateOptionsRef.current && !advancedUpdateOptionsRef.current.contains(event.target)) { const toggleButton = document.getElementById('advanced-update-toggle'); if (toggleButton && toggleButton.contains(event.target)) return; setShowAdvancedUpdateOptions(false); } } if (showAdvancedUpdateOptions) document.addEventListener("mousedown", handleClickOutside); else document.removeEventListener("mousedown", handleClickOutside); return () => document.removeEventListener("mousedown", handleClickOutside); }, [showAdvancedUpdateOptions]);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8 mt-4 mb-2">
      ...
      <div className="mb-3 relative" ref={preGameGoalSetterRef}>
        <div className="flex items-center justify-between bg-gray-800/50 p-2 rounded-lg border border-gray-700/50"> {/* Ensure min height */}
          ...
          <div className="flex items-center">
            ...
            <button 
                id="pre-game-focus-toggle"
                onClick={() => setShowPreGameGoalSetter(!showPreGameGoalSetter)} 
                className="p-1.5 bg-gray-700 hover:bg-gray-600 rounded-md text-gray-300 hover:text-orange-300 transition-colors"
                title={showPreGameGoalSetter ? "Close Focus Setter" : "Set Pre-Game Focus"}
            >
          ...
          <div className="absolute top-full left-0 mt-1 z-30 bg-gray-800 p-3 rounded-lg shadow-xl border border-gray-700 w-full sm:w-auto min-w-[300px] space-y-2">
            ...
            {(goalTemplates && goalTemplates.length > 0) && (
              ...
              >
                ...
                {goalTemplates.map(t => <option key={t.id} value={t.id.toString()}>{t.title} ({t.category || 'General'})</option>)}
              </select>
            )}
