...
const LiveGamePlayer = ({ player, rankInfo, riotId, isTrackedPlayer, getChampionImage, getSummonerSpellImage, getRuneImage, runesDataFromDDragon, runesMap, teamId, championDetails, ddragonVersion }) => {
  ...
  // Create a ref for the player card container
  const playerCardRef = useRef(null);

  const containerClasses = `relative flex flex-col justify-between pt-10 pb-0 rounded-lg transition-all overflow-visible ${isTrackedPlayer ? "bg-gray-800/90" : "bg-gray-800/50"} w-full min-h-[220px]`;
  ...
  return (
    <div ref={playerCardRef} className={containerClasses}>
      {" "}
      {/* Assign the ref here */}
      ...
      <div className="absolute inset-x-0 top-0 h-auto z-10">
        ...
        >
          ...
          <div className="absolute top-2 right-2 flex gap-1 cursor-pointer z-20">
            <div className="w-6 h-6 rounded-full bg-gray-900 flex items-center justify-center border border-black/10">{getRuneImage(player.perks.perkIds[0]) ? <img src={getRuneImage(player.perks.perkIds[0])} alt="Primary Rune" className="w-5 h-5" /> : <div className="w-5 h-5 rounded-sm bg-gray-700/50 flex items-center justify-center text-xs text-gray-500">K?</div>}</div>
            <div className="w-6 h-6 rounded-full bg-gray-900 flex items-center justify-center border border-black/10">{getRuneImage(player.perks.perkSubStyle) ? <img src={getRuneImage(player.perks.perkSubStyle)} alt="Secondary Tree" className="w-5 h-5 p-0.5" /> : <div className="w-5 h-5 rounded-sm bg-gray-700/50 flex items-center justify-center text-xs text-gray-500">S?</div>}</div>
          </div>
        </RuneDisplay>
      </div>
