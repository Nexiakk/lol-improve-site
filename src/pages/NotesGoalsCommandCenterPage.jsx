...
import { Target, Edit3, BarChart3, Zap, PlusCircle, ListChecks, TrendingUp, XCircle, Loader2, AlertTriangle, Activity, CalendarClock, Lightbulb, MessageSquareWarning, Smile, MehIcon, SmilePlus } from "lucide-react";
...
const GoalTemplateForm = memo(({ onAddTemplate, isSaving }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 p-3 bg-gray-700/50 rounded-md">
      ...
      <div>
        <label htmlFor="templateTitle" className="text-xs text-gray-400 block mb-1">
          Title
        </label>
      ...
      <div>
        <label htmlFor="templateDesc" className="text-xs text-gray-400 block mb-1">
      ...
      <div>
        <label htmlFor="templateCategory" className="text-xs text-gray-400 block mb-1">
          Category (Optional)
        </label>
...
function NotesGoalsCommandCenterPage() {
  ...
  const isMatchReviewed = (match) => {
    const hasNewGoals = Array.isArray(match.goals) && match.goals.length > 0 && match.goals.some((g) => g.text && g.text.trim() !== "");
    return (
      hasNewGoals || // Check for new multi-goal format
      (match.mainGoal && match.mainGoal.trim() !== "") || // Fallback for old format
  ...
  const fetchDataAndNudges = useCallback(async () => {
    ...
    try {
      ...
      matches.forEach((match, index) => {
        ...
        // Old single-goal format (for backward compatibility)
        else if (match.mainGoal && match.mainGoal.trim() !== "") {
          totalGoalsSetCount++;
        ...
        <div className="lg:col-span-2 space-y-6 lg:space-y-8">
          <Section title="Performance Statistics" icon={BarChart3} isLoading={isLoadingData} error={pageError.includes("statistics") ? pageError : ""}>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 text-sm">
              <div className="bg-gray-700/30 p-3 rounded-md sm:col-span-1 xl:col-span-1">
                <h4 className="font-semibold text-orange-300 mb-1 flex items-center">
              ...
              <div className="bg-gray-700/30 p-3 rounded-md sm:col-span-2 xl:col-span-1">
                <h4 className="font-semibold text-orange-300 mb-1 flex items-center">
                  ...
                  Review Consistency (Last 7 Days)
                </h4>
                {stats.reviewConsistencyData && stats.reviewConsistencyData.length > 0 ? (
                  ...
                  <div className="text-xs text-gray-400 italic h-[50px] flex items-center justify-center">No review data for the last 7 days.</div>
                )}
              </div>
          ...
          <Section title="Gentle Nudges" icon={Zap} isLoading={isLoadingData} error={""}>
            {nudges.length > 0 ? (
              ...
              <p className="text-gray-500 text-sm italic">{isLoadingData ? "Checking for nudges..." : "All caught up! No nudges for now."}</p>
            )}
          </Section>
        </div>
      </div>
