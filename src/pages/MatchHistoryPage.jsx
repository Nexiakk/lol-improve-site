...
const MATCH_COUNT_PER_FETCH = 20; // Default for general updates
const HISTORICAL_MATCH_COUNT_PER_FETCH = 100; // For date-ranged updates
...
function MatchHistoryPage() {
    ...
    // --- Account and Match Data Fetching ---
    ...
    const fetchAllMatchesFromDb = useCallback(async () => {
        if (trackedAccounts.length === 0 && !isLoadingAccounts) {
            setAllMatchesFromDb([]); setIsLoadingMatches(false); return;
        }
    ...
    const handleFilterChange = useCallback((e) => {
        ...
        if (name === "dateRangeStart" || name === "dateRangeEnd") {
            setFilters(prev => ({
                ...prev,
                dateRange: {
                    ...prev.dateRange,
                    [name === "dateRangeStart" ? "startDate" : "endDate"]: value,
                }
            }));
        } else {
    ...
    const handleDatePresetFilter = useCallback((preset) => {
        ...
        let startDate = '';
        let endDate = today.toISOString().split('T')[0]; // Default end date to today
    ...
    const handleUpdateFetchDateChange = useCallback((e) => {
        const { name, value } = e.target;
        setUpdateFetchDates(prev => ({...prev, [name]: value }));
    }, []);
    ...
    const filteredMatches = useMemo(() => {
        return allMatchesFromDb.filter(match => {
            if (filters.patch && match.gamePatchVersion !== filters.patch) return false;
            ...
            if (filters.withPlayer) {
                ...
                const foundPlayer = match.allParticipants?.some(p =>
                    (p.riotIdGameName?.toLowerCase() === name && p.riotIdTagline?.toLowerCase() === tag) ||
                    p.summonerName?.toLowerCase() === name // Fallback for older data or if Riot ID isn't primary
                );
    ...
    // --- Pagination and Grouping Logic ---
    useEffect(() => { // Adjust current page if it goes out of bounds due to filtering
    ...
    useEffect(() => { // Group matches for current page display
        if (filteredMatches.length === 0) {
            setGroupedMatches({}); return;
        }
    ...
    const handleUpdateAllMatches = async (customStartDate, customEndDate) => {
        ...
        // Convert custom dates to epoch seconds if provided
        ...
        if (customStartDate) {
            ...
            countToFetch = HISTORICAL_MATCH_COUNT_PER_FETCH; // Use larger count for historical fetches
        } else {
            // Default: last 2 weeks if no custom start date
        ...
        for (let i = 0; i < trackedAccounts.length; i++) {
            ...
            try {
                ...
                // Always include queue filter unless fetching very old history where queue might not be known/relevant
                ...
                if (matchIdsFromApi.length === 0) {
                    ...
                    if (!customStartDate && !customEndDate) { // Only update lastUpdated for general updates
                ...
                for (const matchId of matchIdsFromApi) {
                    // For historical fetches, we might process more than the usual limit
                ...
                if (!customStartDate && !customEndDate) { // Only update lastUpdated for general, non-historical updates
                    await db.trackedAccounts.update(account.id, { lastUpdated: new Date().getTime() });
                }
            } catch (err) {
    ...
    const handleSaveNotes = async (matchIdToSave, newNotes, newGoals) => {
        if (!selectedMatchForNotes || !matchIdToSave) { setError("Error: Cannot save notes. Missing match data."); return; }
        setIsSavingNotes(true);
        try {
            await db.matches.update(matchIdToSave, { notes: newNotes, goals: newGoals });
            setAllMatchesFromDb(prevMatches => prevMatches.map(m => m.matchId === matchIdToSave ? { ...m, notes: newNotes, goals: newGoals, isNew: false } : m));
            setSelectedMatchForNotes(prev => prev && prev.matchId === matchIdToSave ? {...prev, notes: newNotes, goals: newGoals} : prev);
        } catch (err) { console.error("Error saving notes to Dexie:", err); setError("Failed to save notes."); }
    ...
    const getRuneImage = (runeId) => {
        ...
        if (!runeInfo || !runeInfo.icon) { console.warn(`Rune data or icon missing for ID: ${runeId}`); return null; }
    ...
    const summaryMatches = useMemo(() => { // Use filtered matches for summary, but cap at summary prop count
        return filteredMatches.slice(0, GAMES_FOR_SUMMARY_PROP);
    }, [filteredMatches]);
    ...
    return (
        <div className="flex flex-1 overflow-hidden h-[calc(100vh-4rem)]">
            ...
            >
                <MatchHistoryHeader
                    filteredMatches={summaryMatches} // Pass capped filtered matches for summary
                    allMatches={allMatchesFromDb} // Pass all DB matches for filter options
                ...
                {!isLoadingMatches && Object.keys(groupedMatches).length > 0 && (
                    <div className="space-y-3 max-w-4xl mx-auto px-4 sm:px-6 md:px-8 pb-8">
                        {Object.entries(groupedMatches).map(([dateKey, matchesOnDate]) => (
                            <div key={dateKey}>
                                <h2 className="text-lg font-semibold text-gray-100 pb-1.5">
                                    {dateKey}
                                </h2>
                                <div className="space-y-1">
                                    {matchesOnDate.map(match => {
                                        ...
                                        let primaryPerkId = null; let subStyleId = null; let primaryRuneImg = null; let subStyleImgPath = null;
                                        if (match.perks && match.perks.styles && Array.isArray(match.perks.styles) && Object.keys(runesMap).length > 0 && ddragonVersion) {
                                            ...
                                            if (primaryStyleInfo?.selections?.[0]?.perk) { primaryPerkId = primaryStyleInfo.selections[0].perk; primaryRuneImg = getRuneImage(primaryPerkId); }
                                            if (subStyleInfo?.style) { subStyleId = subStyleInfo.style; subStyleImgPath = getRuneImage(subStyleId); }
                                        }
                                                    ...
                                                    <div className="flex flex-1 items-stretch p-3 ml-1">
                                                        ...
                                                        <div className="flex items-center justify-center space-x-1.5 flex-shrink-0 mx-1">
                                                            ...
                                                            <div className="relative">
                                                                ...
                                                                ) : ( <div className="w-12 h-12 bg-gray-700/50 rounded-md flex items-center justify-center border border-gray-600 shadow-md"> <ListChecks size={20} className="text-gray-500" /> </div> )}
                                                            </div>
                                                        </div>
                                                        <div className="w-px bg-gray-700/60 self-stretch mx-3"></div>
                                                        {/* Spells, Runes, Items Block */}
                                                            ...
                                                            <div className="flex space-x-1">
                                                                ...
                                                                <div className="flex flex-col space-y-0.5">
                                                                    <div className="w-6 h-6 bg-black/20 rounded flex items-center justify-center border border-gray-600/50 p-0.5">
                                                                        {primaryRuneImg ? (<img src={primaryRuneImg} alt={runesMap[primaryPerkId]?.name || "Keystone"} className="w-full h-full object-contain" onError={(e) => { e.target.style.display = 'none'; const parent = e.target.parentNode; if(parent) parent.innerHTML = '<div class="w-full h-full rounded-sm bg-gray-700/50 flex items-center justify-center text-xs text-gray-500">K?</div>'; }} />) : <div className="w-full h-full rounded-sm bg-gray-700/50 flex items-center justify-center text-xs text-gray-500">K?</div>}
                                                                    </div>
                                                                    <div className="w-6 h-6 bg-black/20 rounded flex items-center justify-center border border-gray-600/50 p-0.5">
                                                                        {subStyleImgPath ? (<img src={subStyleImgPath} alt={runesMap[subStyleId]?.name || "Secondary Tree"} className="w-full h-full object-contain" onError={(e) => { e.target.style.display = 'none'; const parent = e.target.parentNode; if(parent) parent.innerHTML = '<div class="w-full h-full rounded-sm bg-gray-700/50 flex items-center justify-center text-xs text-gray-500">S?</div>'; }} />) : <div className="w-full h-full rounded-sm bg-gray-700/50 flex items-center justify-center text-xs text-gray-500">S?</div>}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div className="flex flex-col space-y-0.5">
