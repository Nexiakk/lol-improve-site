...
} from 'lucide-react'; // Removed RefreshCw and Loader2 as they are in MatchHistoryHeader
...
import MatchHistoryHeader from '../components/MatchHistoryHeader'; // Import the new header component
import {
    getContinentalRoute,
    delay,
    timeAgo,
...
function MatchHistoryPage() {
  ...
  const [isUpdatingAllMatches, setIsUpdatingAllMatches] = useState(false); // State remains here
  const [isSavingNotes, setIsSavingNotes] = useState(false);
  const [updateProgress, setUpdateProgress] = useState(''); // State remains here for the header
  ...
  useEffect(() => {
    ...
    const groups = matchesForCurrentPage.reduce((acc, match) => {
      ...
      if (dateObj.getFullYear() === currentYear) {
        dateKey = dateObj.toLocaleDateString('en-US', { month: 'long', day: 'numeric' });
      } else {
        dateKey = dateObj.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
      }
        ...
        for (const matchId of matchIdsFromApi) {
          ...
          if (matchDetail.info && matchDetail.info.gameVersion) {
              ...
              if (versionParts.length >= 2) {
                  gameSpecificPatch = `${versionParts[0]}.${versionParts[1]}`; 
              } else {
  ...
  const matchesForSummary = useMemo(() => {
    return allMatchesFromDb.slice(0, GAMES_FOR_SUMMARY_PROP);
  }, [allMatchesFromDb]);
  ...
  return (
    <div className="flex flex-1 overflow-hidden h-[calc(100vh-4rem)]"> 
        ...
        >
            {/* Match History Header (Summary, Update Button, Filters placeholder) */}
            <MatchHistoryHeader
            ...
            {/* Error messages specific to match list loading, not general update progress */}
            {error && !isUpdatingAllMatches && ( // Only show general page error if not in update mode
